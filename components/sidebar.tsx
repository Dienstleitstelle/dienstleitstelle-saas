...
const NAV: NavItem[] = [
  ...
  { section: 'Übersicht', label: 'Meine Schichten', href: '/meine-schichten', icon: '⭐', rollen: ['mitarbeiter'] },
  { section: 'Übersicht', label: 'Urlaubsplan',   href: '/urlaubsplan',     icon: '🗓️', rollen: ['admin','leitung','mitarbeiter'] },
  ...
  { section: 'Verwaltung', label: 'Abwesenheit',  href: '/abwesenheit',     icon: '🏖️', rollen: ['admin','leitung'] },
  { section: 'Verwaltung', label: 'Zeiterfassung',href: '/zeiterfassung',   icon: '⏱️', rollen: ['admin','leitung'] },
  ...
  return (
    <>
      ...
      <div className="md:hidden flex items-center justify-between px-4 h-12 border-b border-border1 bg-bg1 sticky top-0 z-40">
        <button onClick={() => setOpen(!open)} className="text-text1 text-xl" aria-label="Menü">
          ☰
        </button>
      ...
      >
        <div className="p-4 border-b border-border1">
          ...
          <div className="text-[11px] text-text3 mt-1 truncate" title={firma}>
            {firma}
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-2 py-2">
          {sections.map((sec) => (
            <div key={sec} className="mb-2">
              <div className="text-[9px] font-bold uppercase tracking-wider text-text3 px-2 pt-3 pb-1">
                {sec}
              </div>
              {items
                ...
                .map((it) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`
                        flex items-center gap-2 px-3 py-2 rounded-lg text-[13px] mb-0.5
                        ${active
                          ? 'bg-[var(--accent-dim2)] text-accent font-semibold'
        ...
        <div className="border-t border-border1 p-2 space-y-2">
          <div className="flex items-center gap-2 p-2 rounded-lg bg-bg2 border border-border1">
            ...
            <div className="min-w-0">
              <div className="text-[12px] font-medium text-text1 truncate">
                {vorname} {nachname}
              </div>
